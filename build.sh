...
# Lib and Tool
MBEDTLS_DIR="${ATF_DIR}/mbedtls-3.6.6"
...
check_environment() {
    ...
    if [ ! -d "${MBEDTLS_DIR}" ]; then
        local MBEDTLS_ZIP="${SCRIPT_DIR}/dl/mbedtls-0bebf8b8c7f07abe3571ded48a11aa907a1ffb20.zip"
        if [ -f "${MBEDTLS_ZIP}" ]; then
            warn "mbedtls not found, automatically extracting from dl/..."
            unzip -qo "${MBEDTLS_ZIP}" -d /tmp/
            mv /tmp/mbedtls-0bebf8b8c7f07abe3571ded48a11aa907a1ffb20 "${MBEDTLS_DIR}"
        fi
    fi
