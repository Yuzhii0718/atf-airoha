...
static uint32_t msdc_ck = (MSDC_SRC_CK / 2);
static uint32_t msdc_bus_width = MSDC_BUS_WIDTH;
...
static void msdc_fifo_write(struct msdc_host *host, const uint8_t *buf, uint32_t size)
{
	const uint32_t *wbuf;
...
static int mtk_mmc_write(int lba, uintptr_t buf, size_t size)
{
	...
	if (size > xfer_size) {
		ERROR("MSDC: Write data size exceeds prepared size\n");
		return -EINVAL;
	}
	...
	while (1) {
		...
		if (status & MSDC_INT_DATCRCERR) {
			ERROR("MSDC: CRC error occured while writing data\n");
			ret = -EIO;
			break;
		}

		if (status & MSDC_INT_DATTMO) {
			ERROR("MSDC: Timeout occured while writing data\n");
			ret = -ETIMEDOUT;
			break;
		}
		...
		chksz = MIN(size, (uint32_t)MSDC_FIFO_SIZE);
		if (MSDC_FIFO_SIZE - msdc_fifo_tx_bytes(host) >= chksz) {
			mmio_write_32((uintptr_t)&host->base->sw_dbg_sel, 0x7);
...
static const struct msdc_compatible arht_compat = {
	.clk_div_bits = 12,
	.pad_tune0 = true,
	.async_fifo = true,
	.data_tune = true,
...
int mtk_mmc_init(void)
{
	...
	int ret, idx_msdc, idx_bus_width;
	uint32_t __msdc_ck[] = {
		25 * 1000 * 1000,	//25MHz
		125 * 100 * 1000,	//12.5MHz
		500 * 1000,			//500KHz
		200 * 1000,			//200KHz
		100 * 1000,			//100KHz
	};
	uint8_t __bus_width[] = {MMC_BUS_WIDTH_1};

	host->base = (struct mtk_sd_regs *)MSDC0_BASE;
	host->dev_comp = &arht_compat;

	host->src_clk_freq = MSDC_SRC_CK;
	host->timeout_ns = 100000000;
	host->timeout_clks = 3 * 1048576;

	mtk_mmc_device_info.mmc_dev_type = MMC_IS_EMMC;

	for(idx_msdc = 0; idx_msdc < (sizeof(__msdc_ck)/sizeof(__msdc_ck[0])); idx_msdc++) {
		msdc_ck = __msdc_ck[idx_msdc];
		for(idx_bus_width = 0; idx_bus_width < (sizeof(__bus_width)/sizeof(__bus_width[0])); idx_bus_width++) {
			msdc_bus_width = __bus_width[idx_bus_width];
			ret = mmc_init(&mtk_mmc_ops, msdc_ck, msdc_bus_width, 0, &mtk_mmc_device_info);
			if(ret == 0) {
				INFO("AIROHA eMMC init success.\n");
				return ret;
			}
		}
	}
